...
import {onSnapshot,  addDoc, collection, getFirestore, serverTimestamp,query,orderBy
...
function App() {
  ...
  useEffect(() => {
    ...
    const unsubscribeForMeesages= onSnapshot(q,(snap)=>{
    ...
    <Box bg={"red.50"}>
      {
        ...
        <VStack h={"full"} paddingY={"4"}>
          <Button colorScheme={"red"} w="full" onClick={signOutHandler}>
            Log Out
          </Button>
          <VStack h={"full"} w={"full"} overflowY={"auto"} css={{"&::-webkit-scrollbar" :{
            ...
            {
              messages.map((item)=>(
                <Message key={item.id}  user={item.uid===user.uid ? "me": "other"} 
                 text={item.text} 
                 uri={item.uri}/>
              ))
            }
